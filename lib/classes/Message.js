...
class Message {
  ...
  async isBotAdmin() {
    ...
    const botParticipant = metadata.participants.find((p) => {
      ...
      return (
        ...
        participantId === this.client.getSocket().user?.id ||
        participantNumber === botNumber ||
        p?.id === this.client.getSocket().user?.id
      );
    });
    ...
    const ownerJid = jidNormalizedUser(metadata.owner || "");
    const ownerNumber = ownerJid.split("@")[0];
    return ownerJid === botJid || ownerNumber === botNumber;
  }
  ...
  async isSenderAdmin() {
    ...
    const senderJid = jidNormalizedUser(this.sender || "");
    const senderNumber = senderJid.split("@")[0];
    ...
    const senderParticipant = metadata.participants.find((p) => {
      ...
      const participantNumber = participantId.split("@")[0];
      return (
        participantId === senderJid ||
        participantId === this.sender ||
    ...
    const ownerJid = jidNormalizedUser(metadata.owner || "");
    const ownerNumber = ownerJid.split("@")[0];
    return ownerJid === senderJid || ownerNumber === senderNumber;
  }
  ...
  /**
   * Update group settings (lock/unlock messaging)
   * @param {string} setting - 'announcement' (admin-only) or 'not_announcement' (everyone)
   */
  async groupSettingUpdate(setting) {
    if (!this.isGroup) return null;
    return await this.client.getSocket().groupSettingUpdate(this.jid, setting);
  }
  ...
  async groupInviteCode() {
    if (!this.isGroup) return null;
    return await this.client.getSocket().groupInviteCode(this.jid);
  }
  ...
  async groupRevokeInvite() {
    if (!this.isGroup) return null;
    return await this.client.getSocket().groupRevokeInvite(this.jid);
  }

  /**
   * Update group subject (name)
   */
  async groupUpdateSubject(subject) {
    if (!this.isGroup) return null;
    return await this.client.getSocket().groupUpdateSubject(this.jid, subject);
  }

  /**
   * Update group description
   */
  async groupUpdateDescription(description) {
    if (!this.isGroup) return null;
    return await this.client
      .getSocket()
      .groupUpdateDescription(this.jid, description);
  }
}
