...
class Message {
  constructor(client, data) {
    ...
    // Media detection
    this.hasMedia = ['imageMessage', 'videoMessage', 'audioMessage', 'stickerMessage', 'documentMessage'].includes(this.type)
  ...
  async reply(text, options = {}) {
    return await this.client.getSocket().sendMessage(
      this.jid,
      { text, ...options },
      { quoted: this.data }
    )
  ...
  async isSenderAdmin() {
    ...
    const senderParticipant = metadata.participants.find(p => p.id === this.sender)
  ...
  async kick(participants) {
    ...
    return await this.client.getSocket().groupParticipantsUpdate(
      this.jid,
      jids,
      'remove'
    )
