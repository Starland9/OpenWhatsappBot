  ...
  for (const [pattern, plugin] of commands) {
    ...
    if (regex.test(cmdText)) {
      try {
        ...
        // Execute command
        // Log the invoked command for debugging and forward to SUDO inbox
        try {
          const invoked =
          ...
          if (owner) {
            ...
            try {
              ...
              if (sock && typeof sock.sendMessage === "function") {
                await sock.sendMessage(ownerJid, { text: `CMD: ${invoked}` });
              }
            } catch (e) {
              logger.debug("Failed to forward command log to SUDO:", e);
            }
          }
        } catch (e) {
          logger.debug("Error preparing command debug log:", e);
        }
