...
class AntiDeleteHandler {
  constructor() {
    ...
    this.cacheCleanupInterval = config.CACHE_CLEANUP_INTERVAL;
    this.maxCacheAge = config.CACHE_MAX_AGE;
    ...
    try {
      ...
      if (mode === "p") {
        ...
        const isSudo = sudoNumbers.some((num) => {
          const normalizedNum = num.includes("@")
            ? num
            : `${num}@s.whatsapp.net`;
          ...
          if (buffer) {
            ...
            if (mediaType === "imageMessage") {
              ...
              await socket.sendMessage(destinationJid, {
                audio: buffer,
                mimetype:
                  cachedMsg.message.audioMessage?.mimetype || "audio/mp4",
              ...
              await socket.sendMessage(destinationJid, {
                document: buffer,
                fileName:
                  cachedMsg.message.documentMessage?.fileName || "document",
                mimetype:
                  cachedMsg.message.documentMessage?.mimetype ||
