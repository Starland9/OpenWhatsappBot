...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (action === "start") {
      if (activeGames.has(message.jid)) {
        return await message.reply(
          "⚠️ Une partie est déjà en cours ! Utilisez *.hangman stop* pour l'arrêter.",
        );
      }
    ...
    if (action === "stop") {
      ...
      if (game.startedBy !== message.sender && !isAdmin && !message.isSudo()) {
        return await message.reply(
          "❌ Seul celui qui a lancé la partie ou un admin peut l'arrêter.",
        );
      }
      activeGames.delete(message.jid);
      return await message.reply(
        `🛑 Partie arrêtée. Le mot était : *${game.word}*`,
      );
    }
  ...
  async handleMessage(message) {
    ...
    if (game.guessed.has(letter) || game.wrong.has(letter)) {
      await message.reply(
        `⚠️ La lettre *${letter.toUpperCase()}* a déjà été jouée !`,
      );
      return true;
    }
