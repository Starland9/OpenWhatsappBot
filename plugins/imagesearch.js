  ...
  async execute(message, query) {
    ...
    if (!query) {
      return await message.reply(
        isGif ? getLang("plugins.imagesearch.gif_usage") : getLang("plugins.imagesearch.usage")
      );
    }
    ...
    try {
      ...
      if (isGif) {
        ...
        const GIPHY_API_KEY = config.GIPHY_API_KEY || process.env.GIPHY_API_KEY || "dc6zaTOxFJmzC"; // Public beta key
        ...
        if (!response.data.data || response.data.data.length === 0) {
          await message.react("❌");
          return await message.reply(`❌ ${getLang("plugins.imagesearch.no_gifs")}`);
        }
        ...
        await message.react("✅");
        await message.sendVideo(gifBuffer, `🎬 *${query}*\n\n_via Giphy_`, { gifPlayback: true });

      } else {
        // Search Image using Unsplash
        const UNSPLASH_ACCESS_KEY = config.UNSPLASH_API_KEY || process.env.UNSPLASH_API_KEY;
        
        if (!UNSPLASH_ACCESS_KEY) {
          return await message.reply(`❌ ${getLang("plugins.imagesearch.no_api_key")}`);
        }
        ...
        if (!response.data.results || response.data.results.length === 0) {
          await message.react("❌");
          return await message.reply(`❌ ${getLang("plugins.imagesearch.no_images")}`);
        }
      ...
      if (error.response && error.response.status === 401) {
        await message.reply(`❌ ${getLang("plugins.imagesearch.invalid_api_key")}`);
      } else {
        await message.reply(`❌ ${getLang("plugins.imagesearch.error")}: ${error.message}`);
      }
    }
  },
};
