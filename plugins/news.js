...
module.exports = {
  command: {
    pattern: "news|crypto|stock|bitcoin|weather",
  ...
  async execute(message, query) {
    ...
    try {
      ...
      await message.reply(`❌ ${getLang("plugins.news.error")}: ${error.message}`);
    }
  },
};
...
async function handleCrypto(message, query) {
  ...
  if (!response.data[cryptoId]) {
    await message.react("❌");
    return await message.reply(`❌ ${getLang("plugins.news.crypto_not_found")}`);
  }
  ...
  const cryptoInfo = `${changeEmoji} *${cryptoId.toUpperCase()} ${getLang("plugins.news.price")}*\n\n` +
    `💵 *USD:* $${data.usd.toLocaleString()}\n` +
    `💶 *EUR:* €${data.eur.toLocaleString()}\n` +
    `${changeColor} *24h ${getLang("plugins.news.change")}:* ${priceChange.toFixed(2)}%\n` +
    `📊 *${getLang("plugins.news.market_cap")}:* $${(data.usd_market_cap / 1000000000).toFixed(2)}B\n\n` +
...
async function handleStock(message, query) {
  ...
  const ALPHA_VANTAGE_KEY = config.ALPHA_VANTAGE_KEY || process.env.ALPHA_VANTAGE_KEY;
  
  if (!ALPHA_VANTAGE_KEY) {
    return await message.reply(`❌ ${getLang("plugins.news.no_stock_api_key")}`);
  }
  ...
  const stockInfo = `${changeEmoji} *${symbol} ${getLang("plugins.news.stock_price")}*\n\n` +
    `💵 *${getLang("plugins.news.price")}:* $${price.toFixed(2)}\n` +
    `${changeColor} *${getLang("plugins.news.change")}:* ${change >= 0 ? "+" : ""}${change.toFixed(2)} (${changePercent >= 0 ? "+" : ""}${changePercent.toFixed(2)}%)\n` +
    `📊 *${getLang("plugins.news.volume")}:* ${parseInt(quote["06. volume"]).toLocaleString()}\n` +
    `📅 *${getLang("plugins.news.last_update")}:* ${quote["07. latest trading day"]}\n\n` +
